...
public class StampScript : MonoBehaviour {
    ...
    private void OnTriggerEnter2D(Collider2D p_Col) { if (p_Col.gameObject.tag == "Visa") m_MyBtn.interactable = true;}
    private void OnTriggerExit2D(Collider2D p_Col) { if (p_Col.gameObject.tag == "Visa" && m_MyBtn.interactable) m_MyBtn.interactable = false;}
